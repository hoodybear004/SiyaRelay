...
async def handler(ws):
    ...
    try:
        ...
        if hello.get("type") != "hello":
            await send_json(ws, {"type": "error", "message": "잘못된 접속 요청입니다."})
            return

        group = str(hello.get("group", "")).strip()
        if not group:
            await send_json(ws, {"type": "error", "message": "그룹 키가 없습니다."})
            return
        ...
        async for raw in ws:
            ...
            if kind == "claim_host":
                ...
                if hosts.get(group) is not ws:
                    await send_json(ws, {"type": "error", "message": "현재 호스트가 아니어서 자막을 전송할 수 없습니다."})
                    continue
                await broadcast(group, {
                    "type": "caption",
